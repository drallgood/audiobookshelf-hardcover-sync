...
func TestHandleInProgressBook_MatchingReadNoOpAdvancesState(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			testAudiobook := createTestBook("test-book-matching-read", "Matching Read", "Test Author", "B08N5KWB9H", "9781234567890")
			testAudiobook.Progress.CurrentTime = 300
			testAudiobook.Media.Duration = 1000
			audiobook := toAudiobookshelfBook(testAudiobook)

			userBookID := int64(130)
			editionID := int64(462)
			readID := int64(796)
			manualReadID := int64(797)
			readProgress := tt.hardcoverProgress
			mockClient.On("GetUserBook", mock.Anything, "130").Return(&models.HardcoverBook{
				ID: "book-130", Title: "Matching Read", EditionID: "462", BookStatusID: 2,
			}, nil).Once()
			mockClient.On("GetUserBookReads", mock.Anything, hardcover.GetUserBookReadsInput{
				UserBookID: userBookID,
			}).Return([]hardcover.UserBookRead{
				{ID: readID, ProgressSeconds: &readProgress, EditionID: &editionID},
				// An unrelated physical/manual read has no edition or progress.
				{ID: manualReadID, ProgressSeconds: nil, Progress: 0, FinishedAt: nil},
			}, nil).Once()
			...
			mockClient.AssertNotCalled(t, "DeleteUserBookRead", mock.Anything, mock.Anything)
			mockClient.AssertExpectations(t)
		})
	}
}

func TestHandleInProgressBook_ExistingReadStatusFailureDoesNotAdvanceState(t *testing.T) {
	svc, mockClient := createTestService()

	testAudiobook := createTestBook("test-book-status-failure", "Status Failure", "Test Author", "B08N5KWB9H", "9781234567890")
	testAudiobook.Progress.CurrentTime = 300
	testAudiobook.Media.Duration = 1000
	audiobook := toAudiobookshelfBook(testAudiobook)

	userBookID := int64(124)
	readID := int64(790)
	progressSeconds := 100
	editionID := int64(456)
	statusErr := errors.New("status update failed")

	mockClient.On("GetUserBook", mock.Anything, "124").Return(&models.HardcoverBook{
		ID:           "book-124",
	...
	assert.False(t, exists, "a failed status mutation must not advance existing-read state")
	mockClient.AssertExpectations(t)
}

func TestHandleInProgressBook_NewReadStatusFailureDoesNotAdvanceState(t *testing.T) {
	svc, mockClient := createTestService()

	testAudiobook := createTestBook("test-book-new-status-failure", "New Status Failure", "Test Author", "B08N5KWB8N", "9781234567891")
	testAudiobook.Progress.CurrentTime = 300
	testAudiobook.Media.Duration = 1000
	audiobook := toAudiobookshelfBook(testAudiobook)

	userBookID := int64(125)
	editionID := int64(457)
	statusErr := errors.New("status creation update failed")

	mockClient.On("GetUserBook", mock.Anything, "125").Return(&models.HardcoverBook{
		ID:           "book-125",
		Title:        "New Status Failure",
		EditionID:    "457",
		BookStatusID: 1,
	}, nil).Once()
	mockClient.On("GetUserBookReads", mock.Anything, hardcover.GetUserBookReadsInput{
	...
	mockClient.On("InsertUserBookRead", mock.Anything, mock.MatchedBy(func(input hardcover.InsertUserBookReadInput) bool {
		return input.UserBookID == userBookID && input.DatesRead.ProgressSeconds != nil &&
			*input.DatesRead.ProgressSeconds == 300
	})).Return(12345, nil).Once()
	mockClient.On("UpdateUserBookStatus", mock.Anything, hardcover.UpdateUserBookStatusInput{
		ID:       userBookID,
		StatusID: 2,
	}).Return(statusErr).Once()
...
func TestHandleInProgressBook_CreateNewRead_UsesStateKeyEdition(t *testing.T) {
	...
	mockClient.On("GetUserBook", mock.Anything, "123").Return(&models.HardcoverBook{
		...
		Title:        "Test Book",
		EditionID:    "999",
		BookStatusID: 2, // Different from the edition we encode in stateKey
	}, nil).Once()
...
func TestHandleInProgressBook_RefreshesStaleStartedAtForReread(t *testing.T) {
	...
	// Initial fetch returns the stale unfinished read AND the prior finished
	// audio read needed for stale-reread detection.
...
func TestHandleInProgressBook_SecondChancePreservesStartedAt(t *testing.T) {
	...
	// Initial read fetch: returns both the stale unfinished read AND a finished read
