...
During a sync, `/status` includes a per-profile `snapshot`, and `/summary`
exposes the same outcomes alongside legacy fields. Each processed book has one
outcome: `synced`, `already_current`, `skipped`, `needs_review`, `not_found`,
`failed`, or dry-run `would_sync`. Their counts add up to `processed_so_far`;
