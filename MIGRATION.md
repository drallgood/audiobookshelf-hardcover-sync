...
- **State File**: The application now maintains state between runs in a JSON file
  ...
  - **Edition associations and downgrade limit**: This release stores confirmed audiobook matches from exact regional Audible mappings alongside sync checkpoints. It reads existing sync state schema `3.0` checkpoints without losing their book records and writes sync state schema `4.0`. The previous application release `v4.0.0` reads sync state schemas only through `3.0`, so it cannot use a `4.0` state file; the application does not convert it back. Before upgrading, copy each state file you may need for rollback. Restore a schema `3.0` copy to roll back with the previous checkpoints. If no such copy is available, point the older application at a new or empty state-file path so it can rebuild checkpoints. For CLI configuration use `sync.state_file` or `SYNC_STATE_FILE`; for a web profile use its `sync_config.state_file`. The rejected schema `4.0` file remains untouched, but its saved edition associations cannot be used by the older application.
