...
- **State File**: The application now maintains state between runs in a JSON file
  ...
  - **Edition associations and downgrade limit**: This release stores confirmed audiobook matches from exact regional Audible mappings alongside sync checkpoints. It reads v3 checkpoints without losing their book records and writes state version `4.0`. The previous v4.0.0 application release only reads through state version `3.0`, so it cannot use a `4.0` file; the application does not convert it back. Before upgrading, copy each state file you may need for rollback. Restore a v3 copy to roll back with the previous checkpoints. If no v3 copy is available, point the older release at a new or empty state-file path so it can rebuild checkpoints. For CLI configuration use `sync.state_file` or `SYNC_STATE_FILE`; for a web profile use its `sync_config.state_file`. The rejected `4.0` file remains untouched, but its saved edition associations cannot be used by the older release.
