...
- **Run lifecycle and durable Sync Status history**: Track accepted syncs through queued, running, finalizing, completed, canceled, and failed phases with generation-safe per-profile persistence, truthful partial outcomes, and the newest 10 terminal reports. The streamlined status API and UI provide lightweight polling, clear phase-specific run timestamps, graceful cancellation and shutdown, and isolated per-profile filesystem work. By @Snuffy2. (#186)
