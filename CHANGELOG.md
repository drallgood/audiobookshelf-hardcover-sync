...
- **Hyphenated ISBNs are kept**: An Audiobookshelf ISBN written with hyphens or spaces was dropped from the mismatch export, because ISBN-10 and ISBN-13 were told apart by exact string length. It is now normalized before it is split into ISBN-10 or ISBN-13. Values that are not ISBN-shaped (for example containing letters other than a trailing X) are no longer written to `isbn_10` or `isbn_13`, and a lowercase trailing `x` is written as `X`. By @Snuffy2.
...
- **Ebook items export as ebook editions**: The mismatch JSON export (`edition_*.json` files) now marks an ebook item with `"reading_format": "ebook"` and an `Ebook` edition format, and does not add the audiobook-only `Unabridged` default; `audio_seconds` is `0`. Audiobook items export as before. The `edition` command does not read `reading_format` yet, so importing an ebook export still creates an audiobook-format edition. By @Snuffy2.
