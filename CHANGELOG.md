...
- **Draft a Hardcover edition for a `needs_review` book (API)**: When a sync run marks a book `needs_review` and its record has a Hardcover candidate, an authenticated caller with write permission on the profile can fetch a draft of a new Hardcover edition built from the Audiobookshelf item (`GET /api/profiles/{id}/runs/{runID}/books/{bookID}/edition-draft`). The Hardcover book comes from the run record, a book whose Audiobookshelf item has no ASIN or parseable ISBN is rejected with `409` (a sync could never match an edition created for it), and an ebook-only Audiobookshelf item (an ebook file and no audio) is drafted as an ebook edition with no narrators or audio length. The draft fills the missing ISBN-10 or ISBN-13 form when the other can be derived, and carries `isbn_10_valid`/`isbn_13_valid` with a warning when an ISBN's own check digit is wrong (Hardcover still stores it as given). An audiobook Audiobookshelf marks abridged now drafts as `Abridged` instead of always `Unabridged`, a full Audiobookshelf `publishedDate` is preferred over the year-only fallback, an item tagged with a non-English language gets a warning (the edition still defaults to English and the United States), and Audnex is no longer queried for an ebook's ASIN. Draft preparation carries Audiobookshelf author, narrator and publisher names but does not resolve Hardcover IDs, check Hardcover matches, construct a Hardcover client or send Hardcover requests. Audnex may supply release metadata for an audiobook ASIN. No Hardcover token or scope is needed to preview; the Audiobookshelf token is used only on the server and is never returned. Hardcover may normalize edition-format labels during create. There is no web UI for this yet. By @Snuffy2.

### Changed

- **Edition draft date fallback**: Slow Audnex enrichment leaves time for the draft to finish with Audiobookshelf metadata. Invalid Audnex or Audiobookshelf dates fall through to the next valid source while preserving Audnex, `publishedDate`, then `publishedYear` precedence; caller cancellation is no longer reported as an Audiobookshelf failure.
...
- **Secure on-demand sync details**: Sync Status now loads authorized, book-level results for the current run on demand, while aggregate polling remains lightweight and profile access follows owner, viewer, administrator, and legacy-ownerless permissions. Profile API responses no longer echo Audiobookshelf or Hardcover tokens, regardless of authentication or role; with authentication enabled, pre-existing ownerless profiles are administrator-only and cannot currently be assigned to a user. Each result includes its Audiobookshelf cover, format, and available series position and links to its Audiobookshelf item and, when known, Hardcover; needs-review results also show the candidate's available series position and link the distinct candidate title, Audiobookshelf ASIN to Audible, and Audiobookshelf ISBN to Goodreads. By @Snuffy2. (#185)
- **Run lifecycle and durable Sync Status history**: Track accepted syncs through queued, running, finalizing, completed, canceled, and failed phases with generation-safe per-profile persistence, truthful partial outcomes, startup recovery for interrupted runs, and configurable terminal-report retention (default: 10). The streamlined status API and UI provide lightweight polling, clear phase-specific run timestamps, graceful cancellation and shutdown, isolated per-profile filesystem work, and compatible versioned state files that retain unversioned and v2 checkpoints. By @Snuffy2. (#186)
### Fixed

- **Preserve Audiobookshelf URL on partial profile updates**: Updating a profile's config without an `audiobookshelf_url` (for example a token-only update) cleared the saved endpoint, breaking the next sync against an empty URL. Omitted URLs are now preserved like omitted tokens.
- **Ebook detection and matching**: Audiobookshelf reports `book` as the media type for ebooks as well as audiobooks, so ebooks were shown as "Audiobook" in Sync Status, were never skipped when `include_ebooks` is off, and were matched only against Hardcover audiobook editions. Ebook-only items (an ebook file and no audio) are now recognized from their media content, using the same audio rule Audiobookshelf itself applies (audio files marked excluded do not count), honor `include_ebooks`, and are matched to Hardcover ebook editions by ASIN, ISBN, and title/author candidate details, including the lookups made while enriching mismatch records. **Behavior change:** with the default `include_ebooks: false`, ebook-only items are now skipped; they were previously synced against Hardcover audiobook editions, so users who relied on that should enable `include_ebooks`. By @Snuffy2 (#190)
- **Mismatch export fixes for ISBNs, publishers and ebooks**: The mismatch JSON export (`edition_*.json` files) now keeps an Audiobookshelf ISBN written with hyphens or spaces, which was dropped because ISBN-10 and ISBN-13 were told apart by exact string length: the ISBN is normalized first, a lowercase trailing `x` is written as `X`, and values that are not ISBN-shaped are no longer written to `isbn_10` or `isbn_13`. An ISBN with a wrong check digit is still written as given, and the export now adds `isbn_10_valid` or `isbn_13_valid` (omitted when that ISBN is empty) to say whether its check digit is correct, as Hardcover's fields of the same name do; the `edition` command ignores them. **Behavior change:** a book whose publisher cannot be resolved on Hardcover now exports `publisher_id: 0` instead of `1`, so imports no longer attach an arbitrary publisher, and a publisher that is resolved while the file is written is now kept instead of being dropped from it. Ebook items now export as ebook editions, with `"reading_format": "ebook"`, an `Ebook` edition format, no audiobook-only `Unabridged` default, no `edition_information` unless the record carries a real value (placeholders and debug text are dropped) and `audio_seconds: 0`, including when the Hardcover identifier lookup failed. An audiobook that Audiobookshelf marks abridged now exports `edition_information: "Abridged"` instead of always `"Unabridged"` (the `abridged` flag was not read before); other audiobooks still export `"Unabridged"`. The `edition` command does not read `reading_format` yet, so importing an ebook export still creates an audiobook-format edition. By @Snuffy2 (#195)
...
- **Move logging outside mutexes**: Rate limiter, current-user lookup, sync state, mismatch collection, sync summary, and OIDC state cleanup no longer write log entries while holding a lock, so a slow or blocked log writer cannot stall other requests. Concurrent Hardcover current-user lookups share one in-flight request, which is released, along with the Hardcover request concurrency permit, even if the request panics. By @Snuffy2. (#191)
...
- **Suppress misleading request logs during a daily quota pause**: While an exhausted Hardcover daily quota holds requests until its reset, request-intent logs (GraphQL requests, Hardcover searches, and the sync service's per-book search and read-status messages) are no longer written for requests that are not being sent. The client now acquires its rate-limiter permit before logging an executed GraphQL request. Pacing and admission behavior are unchanged. By @Snuffy2. (#192)
...
- **HandleFinishedBook Read Insert Before Status**: Swapped the order: insert/update reads first, then set status to FINISHED, then clean up blank reads. The old order triggered Hardcover to auto-create a blank finished read row, hiding the real read from the activity stream. (#167)
- **Blank Read Accumulation Across Sync Cycles**: Added shared `deleteBlankReads` helper and wired it into all four status-transition paths (IN_PROGRESS update/read, IN_PROGRESS create, COMPLETED, FINISHED). Reread transitions left auto-created blank reads with no progress/seconds/finished_at. (#167)
- **Rereads Stuck at READ (Update Path)**: The update path unconditionally skipped IN_PROGRESS status updates. Added `hcBook.BookStatusID` check: only skips when already IN_PROGRESS (2). (#167)
- **Stale User Book Cache (6h TTL) Blocking Transitions**: Added `InvalidateByUserBook()` after every `UpdateUserBookStatus` success to prevent stale `BookStatusID: 2` from persisting across cycles. (#167)
- **Rate Limiter Backoff Compounding**: Daily limit check used `r.rate * 4`, compounding exponentially. Fixed with `max(DefaultRate*10, r.rate)` and `backoffUntil` guard. (#167)
- **Rate Limiter Thresholds Lowered**: Daily backoff from 10%→1%, warning from 20%→5%. (#167)

### Changed
- **API Call Reduction in handleInProgressBook**: `GetUserBookReads` calls reduced from 3 to 1 per book by fetching all reads once and reusing throughout the function. Removed second-chance fetch block. (#167)
- **IETF Rate Limit Headers**: Parses new `RateLimit`/`RateLimit-Policy` headers, backward-compatible with legacy `X-RateLimit-*`. Daily limit tracking with automatic slowdown. (#167)
