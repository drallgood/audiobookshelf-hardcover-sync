...
- **Edition creator hardening**: The `edition` command and the shared edition creator now look for an existing edition by ASIN, ISBN-13 and ISBN-10 (also under the converted ISBN form) before creating, and refuse to adopt one that belongs to a different Hardcover book or whose book cannot be confirmed. A reused edition is left untouched, with no cover upload, and `edition create` prints `"existing": true`. The requested `edition_format` is now sent to Hardcover (trimmed; `Audiobook` when empty), where it was previously always `Audiobook`, which also changes the `edition` command. The `edition` command accepts an optional `reading_format` (`audiobook` by default, or `ebook`); `ebook` creates the edition with Hardcover's ebook reading format, an `Ebook` default `edition_format`, and no narrators or audio length, and looks for an existing edition among ebook editions only. A failed cover upload is now reported as `image_error` in the result (the edition is still created, as before). **Behavior change:** a cover is now uploaded only when the downloaded file is a PNG or JPEG (decided from its bytes, not the response `Content-Type`) of at most 15 MB, which are Hardcover's documented formats and the largest size it documents (WebP is not supported); otherwise the edition is still created without a cover and `image_error` says why, where before any image type of any size was sent. The creator can also limit the Audiobookshelf token to a configured base URL (`Creator.SetAudiobookshelfBaseURL`); the `edition` command does not set it. **Behavior change:** the edition creator's HTTP client (shared by the `edition` and `image-tool` commands) no longer copies the request's `Authorization` header onto redirects, so the Audiobookshelf or Hardcover token is no longer forwarded when a request redirects to a different domain (net/http still forwards it to subdomains and other ports of the same host), and is no longer sent on any request that leaves `https`. By @Snuffy2.
