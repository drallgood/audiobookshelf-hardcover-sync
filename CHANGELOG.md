...
### Fixed
- **Finished Books Not Appearing in Activity Stream**: Fixed the order of operations in `HandleFinishedBook` — the status update (to FINISHED) was happening BEFORE the read record insert/update, triggering Hardcover to auto-create a blank read row on the status transition. This blank read competed with or hid the real read entry in the activity stream. Restructured to match `handleInProgressBook`'s proven pattern: insert/update reads first, then update status, then clean up any auto-created blank reads. Added blank-read cleanup after FINISHED status transitions (#167).
- **Incremental Sync Re-Processing All Books Every Cycle**: Fixed `HasProgressSeconds` flag not being set for terminal/skipped-state books (WANT_TO_READ, NOT_FOUND, NO_EDITION, SKIPPED). The incremental check's `NeedsSync` lookup found the base-key state entry but `HasProgressSeconds=false` caused it to always return true, meaning `findBookInHardcover` (1-3 API calls) ran for every book on every sync cycle — not just genuinely changed books. Now all terminal state updates also call `SetHasProgressSeconds` so previously-processed books correctly skip re-processing.
