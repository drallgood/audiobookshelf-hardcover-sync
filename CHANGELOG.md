...
- **Run lifecycle and history in Sync Status**: Sync starts now acknowledge an accepted queued run, and the UI follows canonical queued, running, finalizing, completed, canceled, and failed phases. Partial canceled or failed reports remain available, the newest 10 terminal reports per profile are retained, and last-attempted and last-successful timestamps distinguish dry-run or unsuccessful attempts from real successful syncs.
- **Account for per-run book outcomes**: Account for each attempted Audiobookshelf book with one per-run outcome so processed totals reconcile with the outcome categories. By @Snuffy2 (#183)
- **Current-run sync outcome status**: Aggregate status and exact run-detail responses identify the run, queued time, state, candidate total, and processed count. Each processed item has one of seven mutually exclusive outcomes (`synced`, `already_current`, `skipped`, `needs_review`, `not_found`, `failed`, or dry-run `would_sync`), whose counts reconcile to `processed_so_far`. Needs-review, not-found, and failed records are available immediately by filtering `book_outcomes`. By @Snuffy2 (#184)
...
- **Sync status internals**: Removed obsolete full-status lookup paths, the process-global mismatch collector, unused persisted state fields, and the write-only run-report version. Existing databases still migrate legacy `last_sync` timestamps into canonical attempt metadata.
