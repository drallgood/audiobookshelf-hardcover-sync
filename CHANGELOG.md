...
- **Profile API credential redaction and ownership**: Profile API responses no longer echo Audiobookshelf or Hardcover tokens, regardless of authentication or role. With authentication enabled, pre-existing ownerless profiles are administrator-only; ordinary users cannot list or access them, and no supported claim or reassignment workflow exists. (#185)
