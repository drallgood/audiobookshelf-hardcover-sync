...
- **Edition capability reporting**: Added a profile-authorized, read-only API that reports ebook `insert_edition` and audiobook `upsert_book` capability separately. Configured tokens remain unverified in this step because Hardcover exposes no documented read-only scope check and create handlers are not yet available; unverified status permits a later eligible user-confirmed attempt with a warning. Missing credentials are reported as denied. The forthcoming create handler can use an explicit operation success or insufficient-scope response as evidence. The route makes no mutation probe. By @Snuffy2.
