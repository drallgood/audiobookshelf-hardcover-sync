...
- **Edition creator hardening**: The `edition` command and the shared edition creator now reuse an existing edition (found by ASIN, ISBN-13 or ISBN-10) instead of creating a duplicate, refuse one that belongs to another book, and print `"existing": true` when they reuse one. The requested `edition_format` is now sent instead of always `Audiobook`, and a new optional `reading_format` (`audiobook` or `ebook`) creates ebook editions. **Behavior change:** a cover is uploaded only if it is a PNG or JPEG of at most 15 MB, a failed cover upload is reported as `image_error`, and the `Authorization` header is no longer forwarded to other domains on a redirect (this also applies to `image-tool`). The `edition` and `image-tool` commands now send the Audiobookshelf token only to the configured Audiobookshelf URL, and fall back to the old "URL contains audiobookshelf" check when none is configured. By @Snuffy2.
