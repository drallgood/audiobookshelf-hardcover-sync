...
- **Reduce noisy info-level logging**: Log routine startup, configuration, per-book, per-profile, and rate limiter pacing detail at debug instead of info, since info is the default log level. Lifecycle events, the sync summary, dry-run messages, warnings, and errors are unchanged. By @Snuffy2. (#189)
