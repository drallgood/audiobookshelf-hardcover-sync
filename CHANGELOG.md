...
- **Edition creation no longer adopts another book's edition**: The `edition` command and the shared edition creator now look for an existing audiobook edition by ASIN, ISBN-13 and ISBN-10 (also under the converted ISBN form) before creating, refuse to adopt one that belongs to a different Hardcover book, and no longer upload or attach a cover to an edition they reuse; `edition create` prints `"existing": true` when it reused one. By @Snuffy2.
- **Ebook editions in the `edition` command**: The `edition` command accepts an optional `reading_format` (`audiobook` by default, or `ebook`); `ebook` creates the edition with Hardcover's ebook reading format, an `Ebook` default `edition_format`, and no narrators or audio length, and looks for an existing edition among ebook editions only. Mismatch files exported for ebook items already carry `"reading_format": "ebook"`. Audiobook input creates exactly as before. By @Snuffy2.
- **Requested edition format is sent to Hardcover**: When creating an edition, the requested `edition_format` is now sent to Hardcover (trimmed; still `Audiobook` when empty), where it was previously always `Audiobook`. This also applies to the `edition` command. By @Snuffy2.
