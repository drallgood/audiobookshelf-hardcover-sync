...
- **Move logging outside mutexes**: Rate limiter, current-user lookup, sync state, mismatch collection, sync summary, and OIDC state cleanup no longer write log entries while holding a lock, so a slow or blocked log writer cannot stall other requests. Concurrent Hardcover current-user lookups share one in-flight request, which is released even if the request panics. By @Snuffy2. (#191)
