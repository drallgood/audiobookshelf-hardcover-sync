...
- **Ebook detection and matching**: Audiobookshelf reports `book` as the media type for ebooks as well as audiobooks, so ebooks were shown as "Audiobook" in Sync Status, were never skipped when `include_ebooks` is off, and were matched only against Hardcover audiobook editions. Ebook-only items (an ebook file and no audio) are now recognized from their media content, using the same audio rule Audiobookshelf itself applies (audio files marked excluded do not count), honor `include_ebooks`, and are matched to Hardcover ebook editions by ASIN, ISBN, and title/author candidate details. By @Snuffy2
