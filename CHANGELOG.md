...
### Fixed
- **UI "Books Processed" Counter was showing "Books Synced"**: The status card label read "Books Processed: 53 of 345" but displayed the synced count instead of the actual processed count. The progress bar also used the wrong numerator. Fixed to show `total_books_processed` for the "Processed" label and progress bar, with a separate "Books Synced" line showing the `books_synced` count (#167).
- **Blank Read Accumulation Across Sync Cycles**: Added shared `deleteBlankReads` helper and wired it into all four status-transition paths (IN_PROGRESS update path, IN_PROGRESS create path, COMPLETED path, and FINISHED path in `HandleFinishedBook`). Previously only the create path had cleanup; reread transitions from READ→IN_PROGRESS left auto-created blank reads accumulating with no progress/seconds/finished_at (#167).
- **53 FINISHED Books Re-Syncing Every Cycle**: `HasProgressSeconds` was only set on new entries — existing FINISHED books from before the fix had `false` permanently. Added propagation in `UpdateBook`: when status is `FINISHED`, the flag is set to `true` unconditionally on both the composite-key and base-key entries, even when no progress/status change occurs. This auto-migrates existing state files without a manual migration step (#167).
- **Rereads Stuck at READ (Update Path)**: The update path in `handleInProgressBook` unconditionally skipped IN_PROGRESS status updates. Read-path status transitions (READ→IN_PROGRESS) were silently dropped for books where an existing unfinished read was being updated. Added `hcBook.BookStatusID` check: only skips when already IN_PROGRESS (2), transitions from READ (3) for rereads (#167).
- **Stale User Book Cache Blocking Status Transitions**: Added `InvalidateByUserBook()` to the persistent user book cache (6h TTL) and called it after every `UpdateUserBookStatus` success. Without this, a cached `BookStatusID: 2` from a previous sync could persist across cycles, causing status transitions to be incorrectly skipped (#167).
- **HandleFinishedBook Read Insert Before Status**: Swapped the order in `HandleFinishedBook`: insert/update reads FIRST, THEN set status to FINISHED, THEN clean up blank reads. The old order (status first) triggered Hardcover to auto-create a blank finished read row, which hid the real read from the activity stream. Added blank-read cleanup after the FINISHED transition (#167).
- **API Call Savings**: `HandleFinishedBook` now calls `SetHasProgressSeconds` in its defer block, matching the terminal-state pattern. Was the final missing piece in the incremental-sync chain (#167).

- **Rate Limiter Backoff Compounding**: Fixed the daily limit check using `r.rate * 4` to compute backoff, which compounded exponentially on every response since `r.rate` is the already-adjusted rate. Changed to `max(DefaultRate*10, r.rate)` with a `backoffUntil` guard preventing re-application while already in backoff (#167).
- **Rate Limiter Thresholds Lowered**: Dropped daily limit backoff from 10% (5000 remaining) to 1% (500) and warning from 20% to 5%. The old thresholds triggered slowdowns with half the supporter quota still available (#167).

### Changed
- **API Call Reduction in handleInProgressBook**: Reduced `GetUserBookReads` calls from up to 3 per book to 1 by fetching all reads in a single API call and reusing the result throughout the function. Removed the second-chance fetch block that was duplicating work when the initial query appeared empty, and the stale-reread detection now reuses the shared fetch data instead of making a separate call. This significantly reduces API traffic for the most common code path (#167).
- **Rate Limit Header Handling for Hardcover API v2**: Updated the rate limiter to parse the new IETF RFC-draft `RateLimit`/`RateLimit-Policy` headers introduced in the August 2026 Hardcover API refresh, while maintaining backward compatibility with legacy `X-RateLimit-*` headers. Headers are now processed on every response (not just errors) so the rate limiter can self-throttle proactively. Added daily limit tracking with automatic slowdown when remaining requests drop below 1% (4x rate reduction) or 5% (warning log). Exposed `DailyRemaining()` and `DailyLimit()` getters for callers to skip non-critical operations when approaching the daily cap (#167).
