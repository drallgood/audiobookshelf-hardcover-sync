...
- **Finished read dates for newly matched books**: Create new Hardcover user books as `WANT_TO_READ` before adding the Audiobookshelf-dated finished read, preventing Hardcover from auto-creating a completion dated at sync time. If Audiobookshelf reports a finished book without a completion date, leave its Hardcover reading state unchanged—the same behavior already used for matched books—so it can be retried when a date is available. Dry runs report this case as skipped rather than as a planned finish.
